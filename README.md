...
### 关于 Windows 打包
由于开发机器内存限制（反复出现内存溢出崩溃），无法在本机完成 Windows `.exe` 完整打包。经与课程方沟通确认，采用以下替代方案：

- ✅ **编辑器双开窗口**录制多人联机演示视频
- ✅ 视频完整展示了联机、打怪、计分、胜利全流程
- ✅ 项目源码（Blueprint）完整上传至本仓库

### 关于 Android 打包
因国内网络限制（Google 服务不可用），Android 构建工具链依赖的 Gradle/SDK 组件下载受阻，多次尝试均因网络问题失败。其他测试项已全部验证通过，被迫采用当前交付策略。
